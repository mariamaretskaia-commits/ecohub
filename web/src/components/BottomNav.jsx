...
const TABS = [
  ...
  { id: 'chat', sticker: 'phone', label: 'Чат' },
        ...
        {TABS.map((tab) => {
          ...
          return (
            <button
              ...
              className={`relative flex-1 flex flex-col items-center py-1.5 rounded-2xl transition-all ${
                isActive ? 'bg-mint-100' : ''
              }`}
            >
              <Sticker name={tab.sticker} size={isActive ? 36 : 32} className={isActive ? '' : 'opacity-70'} />
              {tab.id === 'chat' && chatUnread > 0 && (
                <span className="absolute top-0.5 right-[18%] min-w-[18px] h-[18px] px-1 rounded-full bg-mint-600 text-white text-[10px] font-extrabold flex items-center justify-center">
                  {chatUnread > 99 ? '99+' : chatUnread}
                </span>
              )}
              <span className={`text-[11px] font-extrabold mt-0.5 ${isActive ? 'text-mint-700' : 'text-ink/40'}`}>
                {tab.label}
              </span>
            </button>
          );
